...
class RadarBackgroundPainter extends CustomPainter {
  ...
  void _drawBgGlow(Canvas canvas, Offset c, double maxR) {
    final g = Paint()..shader = RadialGradient(colors: [AegisColors.electricBlue.withValues(alpha: 0.03 + 0.02 * sin(pulseValue * pi * 2)), Colors.transparent], stops: [0.3, 1.0]).createShader(Rect.fromCircle(center: c, radius: maxR));
  ...
  void _drawSweep(Canvas canvas, Offset c, double maxR) {
    ...
    final sweepPaint = Paint()
      ..shader = SweepGradient(
        ...
        colors: [
          Colors.transparent,
          AegisColors.electricBlue.withValues(alpha: 0.08),
          AegisColors.electricBlue.withValues(alpha: 0.15),
          AegisColors.electricBlue.withValues(alpha: 0.08),
          Colors.transparent,
        ],
    ...
    final linePaint = Paint()
      ..color = AegisColors.electricBlue.withValues(alpha: 0.12 + 0.06 * sin(pulseValue * pi * 2))
      ..strokeWidth = 1.5
  ...
  void _drawRings(Canvas canvas, Offset c, double maxR) {
    for (int i = 0; i < 4; i++) {
      ...
      final p = Paint()
        ..color = Color.lerp(const Color(0xFF00E5FF), const Color(0xFF0088FF), i / 3)!.withValues(alpha: AegisColors.isLight ? (0.16 + 0.04 * sin(pulseValue * pi * 2 + i * 1.2)) : (0.06 + 0.04 * sin(pulseValue * pi * 2 + i * 1.2)))
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.0 - i * 0.15;
      canvas.drawCircle(c, r, p);
      final gp = Paint()
        ..color = (AegisColors.isLight ? const Color(0xFF0088FF) : const Color(0xFF00E5FF)).withValues(alpha: AegisColors.isLight ? (0.025 * (i + 1)) : (0.01 * (i + 1)))
  ...
  void _drawCrosshairs(Canvas canvas, Offset c, double maxR) {
    final p = Paint()..color = (AegisColors.isLight ? const Color(0xFF0088FF) : const Color(0xFF00E5FF)).withValues(alpha: AegisColors.isLight ? 0.15 : 0.06)..strokeWidth = 0.3;
    ...
    final dp = Paint()..color = (AegisColors.isLight ? const Color(0xFF0088FF) : const Color(0xFF00E5FF)).withValues(alpha: AegisColors.isLight ? 0.08 : 0.03)..style = PaintingStyle.stroke..strokeWidth = 0.3;
    for (int i = 1; i <= 3; i++) {
  ...
  void _drawConnections(Canvas canvas, Offset c, double maxR) {
    ...
    for (final node in nodes) {
      ...
      if (node.status == NodeStatus.sos) {
        for (int i = 0; i < 3; i++) {
          final glow = Paint()..color = AegisColors.sosRed.withValues(alpha: 0.06 + 0.04 * sin(pulseValue * pi * 2))..maskFilter = MaskFilter.blur(BlurStyle.normal, 8 + i * 4);
          canvas.drawLine(c, pos, glow);
        }
        for (int i = 0; i < 3; i++) {
          final pp = Paint()..color = AegisColors.sosRed.withValues(alpha: 0.4 - i * 0.1)..strokeWidth = 2.0;
          final t = ((pulseValue * 3 + i * 0.33) % 1.0);
          canvas.drawCircle(Offset(c.dx + (pos.dx - c.dx) * t, c.dy + (pos.dy - c.dy) * t), 3.0 - i * 0.8, pp);
        }
      }

      final lc = node.color.withValues(alpha: node.status == NodeStatus.offline ? 0.06 : 0.12 + 0.06 * sin(pulseValue * pi * 2 + node.dx));
      ...
      if (node.status == NodeStatus.offline) {
        ...
        canvas.drawCircle(pos, 2.5, Paint()..color = node.color.withValues(alpha: 0.25));
      }
    }
    ...
    for (int i = 0; i < nodes.length; i++) {
      for (int j = i + 1; j < nodes.length; j++) {
        ...
        canvas.drawLine(ap, bp, Paint()..color = const Color(0xFF00E5FF).withValues(alpha: 0.03)..strokeWidth = 0.3);
      }
    }
  }
            ...
            return Stack(
              ...
              children: [
                    ...
                    child: GestureDetector(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Stack(alignment: Alignment.center, children: [
                          if (isSos) ...[
                            Container(width: size + 18, height: size + 18, decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [BoxShadow(color: AegisColors.sosRed.withValues(alpha: 0.2 + 0.15 * sin(_pulseA.value * pi * 2)), blurRadius: 24 + 12 * sin(_pulseA.value * pi * 2), spreadRadius: 6 + 6 * sin(_pulseA.value * pi * 2))])),
                            Container(width: size + 12, height: size + 12, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: AegisColors.sosRed.withValues(alpha: 0.3 + 0.2 * sin(_pulseA.value * pi * 2)), width: 2.5))),
                          ],
                          if (node.isUser)
                            Container(width: size, height: size, decoration: BoxDecoration(gradient: AegisColors.primaryGradient, shape: BoxShape.circle, border: Border.all(color: Colors.white.withValues(alpha: 0.9), width: 2.5), boxShadow: AegisColors.glowBlue), child: Center(child: Icon(Icons.person_rounded, color: Colors.white, size: 22)))
                          else
                            Container(width: size, height: size, decoration: BoxDecoration(color: node.color, shape: BoxShape.circle, border: Border.all(color: isSos ? AegisColors.sosRed : AegisColors.border2, width: 2), boxShadow: [BoxShadow(color: node.color.withValues(alpha: isSos ? 0.5 : 0.25), blurRadius: isSos ? 16 : 10, spreadRadius: isSos ? 4 : 2)]), child: Center(child: Icon(isSos ? Icons.warning_amber_rounded : Icons.sensors_rounded, color: Colors.white, size: 16))),
                        ]),
                        if (node.isUser) ...[
                          SizedBox(height: 6),
                          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.6), borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.white.withValues(alpha: 0.15), width: 0.5)), child: Text('YOU', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: 1.0))),
                        ],
                      ]),
                    ),
                  );
                ...
                if (widget.nodes.length == 1)
                  Positioned(left: c.dx - 100, top: c.dy + maxR * 0.3, child: SizedBox(width: 200, child: Column(mainAxisSize: MainAxisSize.min, children: [Text('Scanning for survivors...', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white.withValues(alpha: 0.6), letterSpacing: 0.5)), SizedBox(height: 16), SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2.5, valueColor: AlwaysStoppedAnimation<Color>(AegisColors.neonGreen.withValues(alpha: 0.6))))]))),
              ],
            );
          }),
        );
